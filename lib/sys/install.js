(function () {
  ...
  var fs = require('fs'),
    ...
    clock = require('clock'),
    interval = clock.interval(250),
    spinner = (require('cli-color/lib/throbber'))(interval),
    S = require('string'),
  ...
  _.extend(installer, /** @exports installer */ {
    ...
    log_progress: function (state) {
      ...
      console.log();
      installer.log({ prefix: '{step}.{task}'.format(state), msg: 'Running... '  }, true);
      interval.start();
      current++;
    },
  ...
  prompt.get('Press Enter to Continue', function(err, result) {
    installer.log_progress({ step: 'sys', task: 'install' });
