      ...
      /***
        ...
        server {
          ...
          location / { 
            ...
            error_pages 502 = @handle_node_down;
          }
  ...
  _.extend(nginx, /** @exports config */ {
    ...
    run: function (options) {
      ...
      if (options.dry !== true) {
        ...
        exec('sudo rm -f {enabled} 2&> /dev/null'.format({ enabled: nginx_enabled_path }));
        exec('sudo rm -f {available} 2&> /dev/null'.format({ available: nginx_conf_path }));

        // write nginx site config file
        fs.writeFileSync(nginx_conf_path, nginx_conf);

        exec('sudo -u www-data ln -s {available} {enabled} 2&> /dev/null'.format({
