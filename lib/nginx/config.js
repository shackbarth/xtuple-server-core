  ...
  _.extend(nginx, /** @exports config */ {
    ...
    run: function (options) {
      ...
      if (options.dry !== true) {
        ...
        exec('sudo rm -f {enabled}'.format({ enabled: nginx_enabled_path }));
        exec('sudo rm -f {available}'.format({ available: nginx_conf_path }));

        // write nginx site config file
        fs.writeFileSync(nginx_conf_path, nginx_conf);

        exec('sudo ln -s {available} {enabled}'.format({
