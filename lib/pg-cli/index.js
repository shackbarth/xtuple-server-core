...
_.extend(exports, /** @exports pg-cli */ {
  ...
  psqlFile: function (options, file) {
    ...
    if (result.status !== 0) {
      throw new Error('psql query failed: '+ cmd.stderr);
    }
  ...
  restore: function (options) {
    ...
    if (result.status !== 0 && !/WARNING: errors ignored on restore: /.test(result.stdout)) {
  ...
  createcluster: function (options) {
    ...
    if (result.status !== 0) {
      log.info('pg-cli createcluster', 'socketdir lstat: %j', fs.lstatSync(options.xt.socketdir));
      throw new Error(JSON.stringify({ cmd: cmd, stderr: result.stderr }));
    }
  ...
  ctlcluster: function (options, action) {
    ...
    log.verbose('pg-cli ctlcluster', 'result: %j', result);

    if (result.status === 1 || result.status > 2) {
      // FIXME this looks weird. I should have documented why only the
      // error codes 0 and 2 are ok
      log.info('pg-cli ctlcluster', 'lstat xt.socketdir: %j', fs.lstatSync(options.xt.socketdir));
      throw new Error(result.stderr);
    }
  ...
  dropcluster: function (options) {
    ...
    if (result.status !== 0) {
      log.info('pg-cli dropcluster', 'socketdir lstat: %j', fs.lstatSync(options.xt.socketdir));
      throw new Error(result.stderr);
    }
