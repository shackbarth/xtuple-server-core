(function () {
  ...
  _.extend(cli, /** @exports cli */ {
    ...
    restore: function (options) {
      var pg_restore = 'sudo -u {xt.name} /usr/lib/postgresql/{pg.version}/bin/pg_restore',
        cmd = pg_restore + ' -U {xt.name} -p {pg.cluster.port} -j {jobs} -d {dbname} ',
        result;

      options.jobs = Math.ceil(os.cpus().length / 2);

      if (fs.lstatSync(options.filename).isDirectory()) {
        cmd += '-Fd {filename}';
      }
      else {
        cmd += '{filename}';
      ...
      result = exec(cmd.format(options));
