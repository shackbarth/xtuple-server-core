  ...
  _.extend(build, /** @exports build */ {
    ...
    run: function (options) {
      var xt = options.xt,
        ...
        build_template = 'sudo node {bin} -i -b {backup} -d {dbname} -c {configjs} 2>&1 | sudo tee -a {logfile}',
        extension_template = 'sudo node {bin} -d {dbname} -e {ext_path} -c {configjs} 2>&1 | sudo tee -a {logfile}',
