(function () {
  ...
  _.extend(database, /** @exports database */ {
    ...
    download: [ 'quickstart', 'demo', 'empty' ],
    assets: [ 'masterref-4.3.0' ],
    ...
    run: function (options) {
      var xt = options.xt,
        ...
        files = !xt.setupdemos ? [ ] : _.map(database.download, function (flavor) {
          ...
          if (!fs.existsSync(wget_format.file)) {
            exec('sudo -g xtuple wget -qO {file} {url}'.format(wget_format));
            exec('sudo -g xtuple chown :xtuple {file}'.format(wget_format));
          }
          return wget_format;
        }),
        maindb_path = path.resolve(options.xt.maindb + '.backup'),
        asset_path = path.resolve(options.installer_dir, 'assets'),
        xtrole_sql = xtrole_template.format(options.pg);

      // schedule asset files for installation
      files.concat(_.compact(_.map(database.assets, function (asset) {
        var file = path.resolve(asset_path, asset + '.backup'),
          exists = fs.existsSync(file);
        
        return exists && {
          file: file,
          flavor: path.basename(asset, '.backup')
        };
      })));
      ...
      if (options.xt.maindb) {
        if (fs.existsSync(maindb_path)) {
          files.push({
            file: maindb_path,
            flavor: path.basename(xt.maindb, '.backup')
          });
        }
        else {
