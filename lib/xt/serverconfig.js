(function () {
  ...
  _.extend(serverconfig, /** @exports serverconfig */ {
    ...
    run: function (options) {
      var xt = options.xt,
        ...
        derived_config_obj = _.extend(sample_obj, {
          ...
          datasource: _.extend(sample_config.datasource, {
            ...
            testDatabase: _.find(xt.database.list, function (db) {
              return _.any([
      ...
      exec('sudo -g xtuple mkdir -p ' + config_path);
      exec('sudo -g xtuple mkdir -p ' + ssl_path);
      exec('sudo -g xtuple mkdir -p ' + log_path);
      ...
      encryptionKey = exec('openssl rand 128 -hex');
      ...
      exec('sudo -g xtuple chown -R :xtuple /etc/xtuple');
      exec('sudo -g xtuple chown -R :xtuple /var/log/xtuple');
      exec('sudo -g xtuple chown -R :xtuple /usr/local/xtuple');

      // non-xtuple users have zero access to any of this stuff
      exec('sudo -g xtuple chmod -R o-rwx /etc/xtuple');
      exec('sudo -g xtuple chmod -R o-rwx /var/log/xtuple');
      exec('sudo -g xtuple chmod -R o-rwx /usr/local/xtuple');
