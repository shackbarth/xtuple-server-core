...
var util = module.exports = {
  ...
  createJob: function (plan, schedule, argv, options) {
    var cmd = ['sudo -u', options.xt.name, 'xtuple-server', plan].concat(argv).join(' ');
    var tab = crontab.load.sync(options.xt.name);
    return tab.create(cmd, schedule, 'created during '+ options.planName);
  },
  ...
  /**
   * Generate the build_app command
  ...
  getDatabaseBuildCommand: function (db, options) {
    var cmd = [
        'cd ', options.xt.usersrc,
        '&& sudo -u ', options.xt.name, './scripts/build_app.js',
  ...
  getExtensionBuildCommand: function (db, options, extension) {
    ...
    var cmd = [
      'cd {xt.usersrc} &&',
      'sudo -u {xt.name} ./scripts/build_app.js',
      '-c {xt.configfile}',
      '-e {extension_path}',
      '-d {dbname}'
