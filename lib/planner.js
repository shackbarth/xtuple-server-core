      ...
      _.each(plan, function (phase) {
        _.each(phase.tasks, function (taskName) {
          try {
            ...
            planner.log({ msg: e.stack.split('\n'), prefix: planner.format_prefix(phase.name, taskName) }, false);
    ...
    execute: _.partial(Q.fcall, function (plan, options) {
      ...
      planner.eachTask(plan, function (task, phase, taskName) {
        ...
        if (phaseOptions.execute !== false) {
          planner.log_progress({ phase: phase.name, task: taskName, msg: 'Running...' });
          options.progress.push({ phase: phase.name, task: taskName });
      ...
      // afterInstall
      planner.eachTask(plan, function (task, phase, taskName) {
        planner.log_progress({ phase: phase.name, task: taskName, msg: 'Finishing...' });
