(function () {
  ...
  _.extend(planner, /** @exports planner */ {
    ...
    install: function (plan, options) {
      ...
      // beforeInstall
      planner.eachTask(plan, function (task, phaseName, taskName) {
        options.quiet || planner.log_progress({ phase: phaseName, task: taskName, msg: 'Before Install...' });
      ...
      planner.eachTask(plan, function (task, phaseName, taskName) {
        ...
        options.quiet || planner.log_progress({ phase: phaseName, task: taskName, msg: 'Install...' });
        task.doTask(options);
      ...
      // afterInstall
      planner.eachTask(plan, function (task, phaseName, taskName) {
        options.quiet || planner.log_progress({ phase: phaseName, task: taskName, msg: 'After Install...' });
